...
class BuiltInFunction(BaseFunction):
  ...
  def execute_sleep(self, exec_ctx):
    ...
    if not isinstance(duration, Number):
      return RTResult().failure(
          RuntimeError(f"Expected a number for sleep duration, got {type(duration).__name__}")
      )
  ...
  def execute_round(self, exec_ctx):
    ...
    if not isinstance(value, Number):
      return RTResult().failure(
          RuntimeError(f"Expected a number for round, got {type(value).__name__}")
      )
  ...
  def execute_num(self, exec_ctx):
    value = exec_ctx.symbol_table.get('value')
    return RTResult().success(Number(str(value)))
  execute_str.arg_names = ['value']
  ...
  def execute_random(self, exec_ctx):
    ...
    if not isinstance(start, Number) or not isinstance(end, Number):
        return RTResult().failure("Arguments must be numbers")

    value = random.randint(start.value, end.value)
    return RTResult().success(Number(value))

  execute_random.arg_names = ['start', 'end']
